...
<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/Untitled-Diagram.drawio-1.png)</figure>You may be asking well how the HighAvaiability nodes loadbalance traffic? There are many load balancing techniques like
...
<div class="wp-block-columns is-layout-flex wp-container-3"><div class="wp-block-column is-layout-flow"><figure class="wp-block-image size-full is-resized">![](/assets/img/wordpress_mig/04/image.png)</figure></div><div class="wp-block-column is-layout-flow"><figure class="wp-block-image size-full is-resized">![](/assets/img/wordpress_mig/04/image-1.png)</figure></div></div>Nginx Reverse Proxies
...
<div class="wp-block-image"><figure class="aligncenter size-full">![](/assets/img/wordpress_mig/04/image-19.png)</figure></div>## HA NODES: Install keepalived, haproxy on both servers
...
</div></div><div class="wp-block-group is-layout-constrained"><div class="wp-block-group__inner-container">```
<pre class="wp-block-code">```
#HA02
vrrp_script chk_haproxy {
  script " killall -0 haproxy"  # check the haproxy process
...
<div class="wp-block-image"><figure class="aligncenter size-full">![](/assets/img/wordpress_mig/04/image-8.png)</figure></div><figure class="wp-block-image">![](blob:https://thetechcorner.sk/112af1f8-2c69-4071-b54c-4061fbde63c3)</figure>Then shutdown the VM or disconnect the network interface and try again

<div class="wp-block-image"><figure class="aligncenter size-full">![](/assets/img/wordpress_mig/04/image-10.png)</figure></div><div class="wp-block-image"><figure class="aligncenter size-full">![](/assets/img/wordpress_mig/04/image-11.png)</figure></div># HA NODES: install haproxy (on both servers)
...
</div></div>Change IP’s, names and passowrd credentials at bottom your needs
...
</div></div>ow create keys and add exchange them between both ha VMs

First generate key

ssh-keygen

<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-4.png)</figure>Now copy the key and add it to the authorized\_keys files of the other VM
...
</div></div>vim ~/.ssh/authorized\_keys
...
</div></div>Now try logging in from HA01 to ha02

ssh root@192.168.100.92

<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-5.png)</figure>Repeat the proces for HA02
...
</div></div><figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-6.png)</figure>Start and enable the service
...
<figure class="wp-block-image size-large">![](/assets/img/wordpress_mig/04/image-7-1024x279.png)</figure>## Nginx Reverse proxy node: install lsyncd (on the master node)
...
</div></div>Again we need to generate ssh keys and exchange them
...
</div></div>root@reverseProxy02:~# systemctl restart lsyncd  
...
<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-12.png)</figure>Start and enable the service
...
<figure class="wp-block-image size-large">![](/assets/img/wordpress_mig/04/image-13-1024x81.png)</figure>At this point we should be able to get some traffic thorugh HA01, HA02 or HAvIP (virtual IP) on port 80 to the nginx proxy behind it

<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-14.png)</figure>But nginx doesnt know how to properly handle the comunication yet, to do that we have to do a few changes
...
</div></div>Create a new configuration file for your site in the /etc/nginx/sites-available directory:
...
</div></div>Save and close the file.
...
</div></div>If there are no errors, reload Nginx on both servers
...
</div></div>Delete ethe default template
...
<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-15.png)</figure>Or curl it from localhost

<figure class="wp-block-image size-full">![](/assets/img/wordpress_mig/04/image-16.png)</figure>But you wouldnt get any answer if you would try it over HA proxies thats stans before the nginx Vms
...
<div class="wp-block-image"><figure class="aligncenter size-full">![](/assets/img/wordpress_mig/04/image-17.png)</figure></div>But if we configured everything correctly we should see trafic cominf from

192.168.100.90

<div class="wp-block-image"><figure class="aligncenter size-full">![](/assets/img/wordpress_mig/04/image-18.png)</figure></div></body></html>