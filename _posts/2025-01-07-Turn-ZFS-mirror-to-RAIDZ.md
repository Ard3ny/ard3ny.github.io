...
## Introduction
Recently, I noticed that I'm running out of space on my ZFS pool on my NAS. I initially set up this pool as a mirror, thinking I would never run out of 8TB of space. Well, here we are, two years later, and I need to add more space.
...
The way zfs mirror works is that we have 2 drives and data is replicated 1:1 betweem them - meaning you can up to 1 drive before losing data (out of 2 drives)    

The way zfs RAIDZ1 works is that we have 3 drives and data is "striped" across all of them parallelly - meaning you can up to 1 drive before losing data (out of 3 drives)  

### How ZFS RAIDZ1 Works
* We use that we can lose 1 drive in our mirror pool, use that drive + new bought one + fake loopback device (I'll explain this later) to create desired new RAIDz pool.      
...
We can get this information in many ways. I like to compare the drive which has partion on it and which doesn't.
...
We can see drive "wwn-0x5000c500f7dc72e5 -> ../../sdg" doesn't have partition as only one.
...
To create raidz pool we need 3x disks. Zfs won't allow us to create pool without supplying them on the inicialization of the pool. To get arround this we can create this mentioned fake looback device with the help of sparse file and loopback devices.

I liked the explanatiojn from the original post: 
...
#### What size should sparse file be
We are going to do some math here, so we know how big (precisly) we need
...
Devide that by 1024.. 8001563222016:1024=7814026584 
And subtract 1 ....7814026584-1=7814026583

This is the size we are going to use "7814026583". But the same math and process applies to all disk sizes.
...
Check the status. We can see both old (pool-01) and the new pool (pool-02)
```
zpool status
  pool: pool-01
 state: ONLINE
...
We need to disable fake loopback device created from sparse file, so the zfs won't actually write data on it.
This command disable the pool-02, unmount the device and delete the sparse file.
```bash
...
We can see the pool-02 has become degrated, but it's still online so we can write data on it!
...
#### Create data sets in the new pool (pool-02) for all the datasets from the old pool (pool-01) which we just listed
```bash
...
#### Rsync the data
For coping the data I've decided to use rsync as it's better variation of cp. But mainly if anything happens, you don't need to start over with the copy process.
...
I wanted to keep the old name, for old the links and stuff, but you can keep the new name if you like.
